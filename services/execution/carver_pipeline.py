...
class CarverPipeline:
    ...
    def run(
        ...
        from services.signals.forecast_scalar import (
        ...
        try:
            from services.signals.fii_flow_signal import get_fii_flow_forecasts
        ...
        try:
            ...
            oi_data = {}
            try:
                from services.market_data.nse_fo_bhavcopy import fetch_fo_oi_data
        ...
        try:
            ...
            from services.execution.pairs_trading_live import scan_all_pairs
        ...
        hmm_snap = None
        try:
            from services.regime.regime_hmm import get_hmm_model, get_hmm_blended_weights
            from services.regime.regime_strategy_mix import REGIME_STRATEGY_WEIGHTS
            hmm = get_hmm_model()
            if hmm._fitted:
                # Build observations from recent NIFTY data
                from services.regime.regime_hmm import prepare_hmm_observations
        ...
        if dynamic_weights is None:
            try:
                from services.regime.regime_strategy_mix import get_regime_aware_forecast_weights
        ...
        if dynamic_weights is None:
            try:
                from services.signals.factor_momentum import get_forecast_weights
        ...
        try:
            from services.risk.strategy_decay import StrategyDecayMonitor
        ...
        try:
            from services.signals.thompson_sampling import ThompsonSamplingBandit
        ...
        try:
            from services.signals.forecast_combiner import compute_rolling_correlations
        ...
        if hmm_snap is not None and hmm_snap.confidence >= 0.5:
            try:
                from services.regime.regime_hmm import markov_signal_filter
        ...
        if self.cfg.apply_cost_filter:
            ...
            from services.risk.cost_speed_limit import filter_by_cost
        ...
        from services.portfolio.instrument_weights import (
        ...
        try:
            if len(active_symbols) >= 5:
                from services.portfolio.hrp_allocator import hrp_instrument_weights
        ...
        try:
            from services.execution.risk_managed_momentum import RiskManagedMomentum
        ...
        from services.risk.position_sizer import compute_position_sizes_batch, PositionSizerConfig
        ...
        try:
            from services.risk.cost_speed_limit import check_forecast_capacity
            capacity_dampened = 0
        ...
        try:
            if len(daily_vols) >= 4:
                ...
                from services.risk.portfolio_vol_monitor import compute_portfolio_volatility
        ...
        try:
            from services.execution.twap_vwap_executor import should_use_algo_execution
            _algo_tagged = 0
            for plan in plans:
        ...
        try:
            ...
            if _run_day % 5 == 0:  # Weekly check (every 5th day of year)
                from services.research.walk_forward import _WF_PARAMS_DIR
                from pathlib import Path
