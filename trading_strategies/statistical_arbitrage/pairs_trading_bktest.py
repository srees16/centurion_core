    ...
    for i in range(bandwidth,len(signals)):
        ...
        if prev_status and not coint_status:           
            if signals.at[signals.index[i-1],'signals1']!=0:
                ...
                signals.loc[signals.index[i:], 'z']=np.nan
                signals.loc[signals.index[i:], 'z upper limit']=np.nan
                signals.loc[signals.index[i:], 'z lower limit']=np.nan
                signals.loc[signals.index[i:], 'fitted']=np.nan    
                signals.loc[signals.index[i:], 'residual']=np.nan
        ...
        if not prev_status and coint_status:
            ...
            signals.loc[signals.index[i]:,'fitted']=model.predict(sm.add_constant(signals['asset1'].iloc[i:])).values
            signals.loc[signals.index[i]:,'residual']=signals['asset2'].iloc[i:].values-signals['fitted'].iloc[i:].values
            
            #normalize the residual to get z stat
            #z should be a white noise following N(0,1)
            signals.loc[signals.index[i]:,'z']=(signals['residual'].iloc[i:].values-np.mean(model.resid))/np.std(model.resid)
                        
            #create thresholds
            #conventionally one sigma is the threshold
            #two sigma reaches 95% which is relatively difficult to trigger
            signals.loc[signals.index[i]:,'z upper limit']=signals['z'].iloc[i]+np.std(model.resid)
            signals.loc[signals.index[i]:,'z lower limit']=signals['z'].iloc[i]-np.std(model.resid)
...
def portfolio(data):
    ...
    #this is no forward bias
    #just ensure we have enough â‚¬â‚¬â‚¬ to purchase shares when the price peaks
