...
def plot(new):
    ...
    '''add some captions'''
    long_idx = newbie.loc[newbie['signals']==1].index[0]
    short_idx = newbie.loc[newbie['signals']==-1].index[0]
    
    long_y = newbie['lower band'][newbie['signals']==1].iloc[0]
    short_y = newbie['lower band'][newbie['signals']==-1].iloc[0]
    
    ax.annotate('Expansion', xy=(long_idx, long_y),
                xytext=(-60, -40), textcoords='offset points',
                fontsize=12, fontweight='bold', color='#563838',
                ha='center',
                bbox=dict(boxstyle='round,pad=0.4', facecolor='white', edgecolor='#563838', alpha=0.9),
                arrowprops=dict(arrowstyle='->', color='#563838', lw=1.5))
    ax.annotate('Contraction', xy=(short_idx, short_y),
                xytext=(60, -40), textcoords='offset points',
                fontsize=12, fontweight='bold', color='#563838',
                ha='center',
                bbox=dict(boxstyle='round,pad=0.4', facecolor='white', edgecolor='#563838', alpha=0.9),
...
def main():
    ...
    # Map to expected format: 'price' and 'date' columns
    df = df.reset_index()
