...
def rsi(data,n=14):
    ...
    down_smma = np.array(smma(down,n))
    up_smma = np.array(smma(up,n))
    rs=np.divide(up_smma, down_smma, out=np.zeros_like(up_smma), where=down_smma!=0)
...
def signal_generation(df,method,n=14):
    ...
    df['rsi']=0.0
    df.loc[df.index[n:], 'rsi']=method(df['Close'],n=14)
...
def plot(new,ticker):
    ...
    ax.plot(new.index, new['Close'], label=ticker)
