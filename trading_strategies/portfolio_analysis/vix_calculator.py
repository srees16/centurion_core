...
'''compute VIX-style volatility for AAPL using yfinance options and treasury data'''
def main():
    ...
    # Arbitrary date range for treasury yield curve rates download
    stdate='2025-01-01'
    eddate='2026-02-20'

    # VIX parameters (matching original Henry Hub convention)
    # timeframe_front/rear: month offsets for near/next term contracts
    options_id=9999
    timeframe_front=2;timeframe_rear=3
    expiration_hour=16;expiration_day=9  # ~3rd Friday for equity options
    num_of_mins_timeframe=timeframe_rear*30*24*60
    num_of_mins_year=365*24*60

    # ================================================================
    # 1. Download AAPL options chain from yfinance
    # ================================================================
    now=datetime.now()
    tradedate=now.strftime('%Y-%m-%d')

    print(f"Computing VIX-style volatility for {ticker_symbol}")
    print(f"Trade date: {tradedate}")

    ticker=yf.Ticker(ticker_symbol)
    exp_dates=ticker.options

    if not exp_dates or len(exp_dates)<2:
        print("Need at least 2 expiration dates for VIX calculation")
        return

    # Determine target months using the same logic as vix_calculator:
    # nearterm = tradedate + 30*timeframe_front days -> frontmonth = 1st of that month
    # Options actually expire in the month BEFORE frontmonth (Henry Hub convention)
    nearterm_date=now+timedelta(days=30*timeframe_front)
    nextterm_date=now+timedelta(days=30*timeframe_rear)

    # Delivery months (used for futures-expirationDate filtering in vix_calculator)
    front_delivery=pd.Timestamp(f'{nearterm_date.year}-{nearterm_date.month}-1')
    rear_delivery=pd.Timestamp(f'{nextterm_date.year}-{nextterm_date.month}-1')

    # Actual option expiration months (one month before delivery)
    front_exp_target=front_delivery-pd.DateOffset(months=1)
    rear_exp_target=rear_delivery-pd.DateOffset(months=1)

    # Find expirations in target months
    def find_exp_in_month(exp_dates,target_year,target_month):
        return [(d,datetime.strptime(d,'%Y-%m-%d'))
                for d in exp_dates
                if datetime.strptime(d,'%Y-%m-%d').year==target_year
                and datetime.strptime(d,'%Y-%m-%d').month==target_month]

    front_candidates=find_exp_in_month(exp_dates,front_exp_target.year,front_exp_target.month)
    rear_candidates=find_exp_in_month(exp_dates,rear_exp_target.year,rear_exp_target.month)

    if not front_candidates or not rear_candidates:
        print(f"No expirations found in target months:")
        print(f"  Front: {front_exp_target.year}-{front_exp_target.month:02d}")
        print(f"  Rear: {rear_exp_target.year}-{rear_exp_target.month:02d}")
        print(f"Available: {list(exp_dates[:15])}")
        return

    # Pick closest to standard monthly expiry (~3rd Friday, around day 18)
    front_exp_str=min(front_candidates,key=lambda x:abs(x[1].day-18))[0]
    rear_exp_str=min(rear_candidates,key=lambda x:abs(x[1].day-18))[0]

    print(f"Front month expiry: {front_exp_str} (delivery: {front_delivery.date()})")
    print(f"Rear month expiry: {rear_exp_str} (delivery: {rear_delivery.date()})")

    # Fetch option chains
    chain_front=ticker.option_chain(front_exp_str)
    chain_rear=ticker.option_chain(rear_exp_str)

    # Build options DataFrame matching expected column format
    rows=[]
    for delivery_month,chain in [(front_delivery,chain_front),
                                  (rear_delivery,chain_rear)]:
            ...
            for _,row in data.iterrows():
                ...
                if price>0:
                    rows.append({
                        ...
                        'futures-expirationDate':delivery_month,
                        'futures-updated':now,
                        'options-updated':now,
                        'options-strikePrice':row['strike'],
                        'options-optiontype':opt_type,
                        'options-priorSettle':price
                    })
    ...
    # ================================================================
    # 2. Download treasury yield curve rates from yfinance
    # ================================================================
    # ^IRX = 13-week T-bill rate (closest proxy for short-term risk-free rate)
    irx=yf.download('^IRX',start=stdate,end=eddate,progress=False)

    if isinstance(irx.columns,pd.MultiIndex):
        irx.columns=irx.columns.droplevel(1)

    if irx.empty:
        print("Could not download treasury rate data (^IRX)")
        return
    ...
    # Build cmt_rate DataFrame using ^IRX as proxy for all maturities
    maturities=['1 Mo','1 Yr','10 Yr','2 Mo','2 Yr','20 Yr','3 Mo',
                '3 Yr','30 Yr','5 Yr','6 Mo','7 Yr']
    cmt_rows=[]
    trade_ts=pd.Timestamp(tradedate)
    trade_date_covered=False

    for date_idx in irx.index:
        rate_value=irx.loc[date_idx,'Close']
        if pd.notna(rate_value):
            ...
            if date_idx.normalize()==trade_ts:
                trade_date_covered=True

    # Ensure trade date is covered (may be missing on weekends/holidays)
    if not trade_date_covered:
        last_rate=float(irx['Close'].dropna().iloc[-1])
        for mat in maturities:
            cmt_rows.append({
                'Date':trade_ts,
                'maturity':mat,
    ...
    cmt_rate=pd.DataFrame(cmt_rows)
    ...
    # ================================================================
    # 3. Generate US federal holidays calendar
    # ================================================================
    cal=USFederalHolidayCalendar()
    holidays=cal.holidays(start='2024-01-01',end='2028-12-31')
    calendar=pd.DataFrame({'DATE':[h.strftime('%Y-%m-%d') for h in holidays]})
    ...
    print(f"\n{ticker_symbol} VIX-style volatility index: {vix:.4f}")
