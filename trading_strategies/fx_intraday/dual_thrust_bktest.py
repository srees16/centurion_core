...
# DATA CONFIGURATION
# ============================================================================
# Modify these paths to point to your data location
DATA_DIR = os.path.join(os.path.dirname(__file__), '..', '..', 'project_support_files', 'data')
DATA_FILE = 'gbpusd.csv'
# ============================================================================
...
def min2day(df,column,year,month,rg):
    ...
    for i in range(1,32):
        ...
        try:
            ...
            memo['open'].append(temp.iloc[0])
            memo['close'].append(temp.iloc[-1])
...
def signal_generation(df,intraday,param,column,rg):
    ...
    signals=df[df.index>=intraday['date'].iloc[rg-1]].copy()
    signals['signals']=0
    signals['cumsum']=0
    signals['upper']=0.0
    signals['lower']=0.0
    ...
    for i in signals.index:
        ...
        if i.hour==12 and i.minute==0:
            ...
            signals['cumsum']=signals['signals'].cumsum()
            signals.at[i,'signals']=-int(signals['cumsum'].loc[i])
...
def plot(signals,intraday,column):
    ...
    #add some captions
    text_time = pd.to_datetime('%s-%s-%s 03:00:00'%(date.year,date.month,date.day))
    plt.text(text_time,signew['upper'].loc[text_time],'Upper Bound',color='#C06C84')
    plt.text(text_time,signew['lower'].loc[text_time],'Lower Bound',color='#C06C84')
