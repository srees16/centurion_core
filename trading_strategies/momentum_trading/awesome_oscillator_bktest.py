...
def signal_generation(df,method,ma1,ma2):
    ...
    signals.loc[signals.index[ma1:], 'macd positions']=np.where(signals['macd ma1'].iloc[ma1:]>=signals['macd ma2'].iloc[ma1:],1,0)
...
def stats(portfolio):
    ...
    awesome_std = np.std(portfolio['awesome return'])
    macd_std = np.std(portfolio['macd return'])
    stats['awesome sharpe']=(portfolio['awesome asset'].iloc[-1]/5000-1)/awesome_std if awesome_std > 0 else 0
    stats['macd sharpe']=(portfolio['macd asset'].iloc[-1]/5000-1)/macd_std if macd_std > 0 else 0
