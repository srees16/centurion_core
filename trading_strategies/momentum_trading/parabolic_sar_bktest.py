...
def parabolic_sar(new):
    ...
    new.at[1, 'trend']=1 if new['Close'][1]>new['Close'][0] else -1
    new.at[1, 'sar']=new['High'][0] if new['trend'][1]>0 else new['Low'][0]
    new.at[1,'real sar']=new['sar'][1]
    new.at[1, 'ep']=new['High'][1] if new['trend'][1]>0 else new['Low'][1]
...
def plot(new,ticker):
    ...
    ax.plot(new.index, new['Close'], lw=3, label='%s'%ticker)
    ax.plot(new.index, new['real sar'], linestyle=':', label='Parabolic SAR', color='k')
...
def main():
    ...
    # Keep only OHLC columns we need
